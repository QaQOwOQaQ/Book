...
setup_idt:
	...
	mov eax,00080000h		;// 将选择符0x0008 置入eax 的高16 位中。    eax为地址
...
; */
;// 每个页表长为4 Kb 字节，而每个页表项需要4 个字节，因此一个页表共可以存放1000h 个，
...
; */
_tmp_floppy_area:
	db 1000h dup(0)		;// 共保留1024 项，每项1 字节，填充数值0 。
...
ignore_int:
	...
	pop eax				;//eax接受返回值  当前eax是int_msg打印的长度
...
setup_paging:	;// 首先对5 页内存（1 页目录+ 4 页页表）清零
	mov ecx,1024*5		;/* 5 pages - pg_dir+4 page tables */   ecx计数器    实际上只清除第一页就完事了
	xor eax,eax
...
;// 的属性标志= 0x00001007 & 0x00000fff = 0x07，表示该页存在、用户可读写。
	mov eax,_pg_dir		;//pg_dir=0 eax=pg_dir
...
;// 设置页目录基址寄存器cr3 的值，指向页目录表。
	...
	mov cr3,eax		;/* cr3 - page directory start */   cr3 保存页表
